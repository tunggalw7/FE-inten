...
@Component({
  selector: "list-tryout-ips",
  templateUrl: "./list-tryout-ips.component.html",
  styleUrls: ["./list-tryout-ips.component.scss"],
})
export class ListTryOutIpsComponent implements OnInit {
  actionSelect: String;
  toForm : FormGroup;
  defaultRole = "All Role";
  listRole = [];
  isLoadData = true;
  baseService = "core/to-ips";
  ...
  async onUpload() {    
    ...
    this.masterService.add3(this.baseService, this.rows).subscribe(
      res => {
          ...
          if (res['success']){                
              this.submitLoading = false;
              this.showToast('top-right', 'success', 'You have successfully uploaded tryout-ips.');
